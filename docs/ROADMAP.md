...
| 12 | Tighten the OTB classical-vs-rapid classifier (or use a curated source) | S | **done** (v0.39.1) |
...
| 16 | **Expand knowledge corpus beyond Capablanca** — deposit at least two more public-domain books per `knowledge/README.md` and the SHOPPING_LIST. Good first targets: Nimzowitsch's *My System*, Tarrasch's *The Game of Chess*, or a Euwe game collection. (Acquisition is a **Claude Cowork** task — book hunting/cleaning — not a Claude Code one.) | M | **in-progress** — 8 more books acquired 2026-06-18; corpus at 20 texts / 2965 chunks |
...
| 23 | **Per-ply material trajectory along engine lines** — annotate captures in `best_pv`/variations so "show the money" is computed, not model-tallied. | M | **done** (v0.39.0) |
...
| 25 | **Multi-move sacrifice detection** — flag a material-down / eval-up window across plies in code (today the model judges it from the trajectory). | M | **done** (v0.38.0) |
...
| 37 | **Scoresheet import — photo → verified PGN** — Claude-vision transcription + legality-gated reconstruction (constraint propagation over python-chess; the model proposes, the rules engine disposes). Developer CLI first, GUI/web later. Motivated by two real hand-transcription casualties on file (2026-07-18): the Rafay `24...Rc8` ambiguity and the Andrija `33...Rd7` illegal ply. Spec: `docs/specs/SCORESHEET_IMPORT.md` (Sonnet-executable, Phases 0–3). | M | todo (spec ready) |
| 38 | **Email PGN intake** — James self-sends games by email; idea: a session-driven (never scheduled — AVG doctrine) intake sweep that reads the Gmail label, extracts + validates PGNs into the library. Near-term, the manual protocol in `Documents\Chess Game Files\CLAUDE.md` covers this (2026-07-18: five backlogged email games were extracted by hand-of-Claude, incl. chess.com archive metadata recovery). Kept open as a feature idea per James. | S | todo (idea; protocol covers today) |
| 39 | **Interactive board editor in the report viewer** — from James's 2026-07-18 report critique (item 4A): let the reader move pieces freely on the replay board (an editable "what if" mode) — explicit edit toggle so drags are deliberate, legality-checked play-out from any report position, and a discard/return-to-game control. The smaller siblings shipped first (steppable engine lines with parenthetical numbering + move sounds + result markers, this same critique's items 2/3/4B/4C). Needs a client-side legality layer (JS move generation, or precomputed per-position move maps kept data-back) — write a spec before building. | L | todo (spec first) |
...
**Recently shipped** (newest first): **v0.39.1** — `_humanize_time_control` now labels increment time controls with their FIDE category (classical/rapid/blitz/bullet) using a 40-moves-per-game estimate; 5 new test assertions. **v0.39.0** — `compute_pv_material_delta`: engine-free net material gain along the best PV; new `pv_material_delta` field; narrator rule grounds "best line wins X pawns" claims in verified data; 6 new tests (481 total). **v0.38.0** — `propagate_sacrifice_windows`: second-pass tagger; `in_sacrifice_window` / `sacrifice_window_origin_ply` / `sacrifice_window_invested` fields; narrator `sacrifice_window` key + silence rule on "down material" framing; 7 new engine-free tests (475 total). **v0.37.0** — `is_prophylaxis`: eighth Tier-B predicate; quiet blockade of advanced enemy pawn's advance square; passed-pawn-blockade flag; 8 new tests. **v0.36.0** — `detect_space_advantage`: pawn-space score, lead ≥4 threshold; 6 new tests. **v0.35.0** — `is_initiative`: consecutive-checking sequence in PV; 7 new tests. **v0.34.0** — `is_zwischenzug`: checking zwischenzug (forgoes capture for a check); 5 new tests. **v0.33.0** — `is_hole` helper + `detect_weak_square`: permanent structural hole with piece occupation; 10 new tests. **v0.32.0** — `is_tempo`: attacks minor/major piece, forced reply addresses attacked square; 8 new tests. **v0.31.0** — `is_compensation`: materially down ≥1.5 pawns, eval near-level or better; 11 new tests. **v0.30.0** — `creates_overloaded`: gates `overloaded_piece` via existing detector; 6 new tests. (Tier-B complete at v0.37.0 — 217 factgate tests, 468 total.)
