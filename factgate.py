...
def has_rook_and_minor_vs_rook(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wr == 1 and (wb + wn) == 1 and wq == 0 and bq == 0 and br == 1 and bb == 0 and bn == 0:
            return "white_rm"
        if br == 1 and (bb + bn) == 1 and bq == 0 and wq == 0 and wr == 1 and wb == 0 and wn == 0:
            return "black_rm"
        return None
    ...
    mover_name = "White" if mover_color == chess.WHITE else "Black"
    if state_after == "white_rm":
        rook_minor_side = "White"
        lone_rook_side = "Black"
    else:
        rook_minor_side = "Black"
        lone_rook_side = "White"

    return True, {
        "rook_minor_side": rook_minor_side,
        "lone_rook_side": lone_rook_side,
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a rook-and-minor vs. lone-rook ending, "
            f"with {rook_minor_side} holding rook plus a minor piece against {lone_rook_side}'s "
            f"lone rook — the extra minor piece gives {rook_minor_side} a sustained advantage "
            f"that {lone_rook_side} must neutralise with active rook play."
        ),
    }
...
def has_queen_vs_rook_and_two_minors(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wq == 1 and wr == 0 and wb == 0 and wn == 0 and bq == 0 and br == 1 and (bb + bn) == 2:
            return "white_queen"
        if bq == 1 and br == 0 and bb == 0 and bn == 0 and wq == 0 and wr == 1 and (wb + wn) == 2:
            return "black_queen"
        return None
    ...
    mover_name = "White" if mover_color == chess.WHITE else "Black"
    if state_after == "white_queen":
        queen_side = "White"
        rook_two_minor_side = "Black"
    else:
        queen_side = "Black"
        rook_two_minor_side = "White"

    return True, {
        "queen_side": queen_side,
        "rook_two_minor_side": rook_two_minor_side,
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a queen vs. rook-and-two-minors imbalance, "
            f"with {queen_side} relying on the queen's long-range power against "
            f"{rook_two_minor_side}'s rook and two minor pieces — the rook-plus-minors "
            f"side typically holds more material, so {queen_side} must create immediate "
            f"activity to compensate for the structural deficit."
        ),
    }
...
def has_rook_and_minor_vs_minor_pair(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wr == 1 and (wb + wn) == 1 and wq == 0 and bq == 0 and br == 0 and (bb + bn) == 2:
            return "white_rm2m"
        if br == 1 and (bb + bn) == 1 and bq == 0 and wq == 0 and wr == 0 and (wb + wn) == 2:
            return "black_rm2m"
        return None
    ...
    mover_name = "White" if mover_color == chess.WHITE else "Black"
    if state_after == "white_rm2m":
        rook_minor_side = "White"
        minor_pair_side = "Black"
    else:
        rook_minor_side = "Black"
        minor_pair_side = "White"

    return True, {
        "rook_minor_side": rook_minor_side,
        "minor_pair_side": minor_pair_side,
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a rook-and-minor vs. minor-pair imbalance, "
            f"with {rook_minor_side} holding rook plus a minor piece against "
            f"{minor_pair_side}'s two minor pieces — the rook gives {rook_minor_side} "
            f"a material edge that grows as the position opens."
        ),
    }
...
def has_queen_and_rook_vs_rook_and_minor(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wq == 1 and wr == 1 and wb == 0 and wn == 0 and bq == 0 and br == 1 and (bb + bn) == 1:
            return "white_qr"
        if bq == 1 and br == 1 and bb == 0 and bn == 0 and wq == 0 and wr == 1 and (wb + wn) == 1:
            return "black_qr"
        return None
    ...
    mover_name = "White" if mover_color == chess.WHITE else "Black"
    if state_after == "white_qr":
        queen_rook_side = "White"
        rook_minor_side = "Black"
    else:
        queen_rook_side = "Black"
        rook_minor_side = "White"
    ...
    return True, {
        ...
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a queen-and-rook vs. rook-and-minor imbalance, "
            f"with {queen_rook_side} holding queen and rook against {rook_minor_side}'s "
            f"rook and minor piece — the extra queen gives {queen_rook_side} a decisive "
            f"material advantage that should convert in an open endgame."
        ),
    }
...
def has_queen_and_minor_vs_rook_and_minor(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wq == 1 and wr == 0 and (wb + wn) == 1 and bq == 0 and br == 1 and (bb + bn) == 1:
            return "white_qm"
        if bq == 1 and br == 0 and (bb + bn) == 1 and wq == 0 and wr == 1 and (wb + wn) == 1:
            return "black_qm"
        return None
    ...
    return True, {
        ...
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a queen-and-minor vs. rook-and-minor imbalance, "
            f"with {queen_minor_side} holding queen and a minor piece against {rook_minor_side}'s "
            f"rook and minor piece — the queen's long-range power gives {queen_minor_side} "
            f"a significant material and positional advantage."
        ),
    }
...
def has_two_rooks_vs_rook_and_minor(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wr == 2 and wb == 0 and wn == 0 and wq == 0 and bq == 0 and br == 1 and (bb + bn) == 1:
            return "white_2r"
        if br == 2 and bb == 0 and bn == 0 and bq == 0 and wq == 0 and wr == 1 and (wb + wn) == 1:
            return "black_2r"
        return None
    ...
    return True, {
        ...
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a two-rooks vs. rook-and-minor imbalance, "
            f"with {two_rook_side} holding two rooks against {rook_minor_side}'s "
            f"rook and minor piece — the rook pair gives {two_rook_side} a material edge "
            f"and superior open-file coordination."
        ),
    }
...
def has_queen_and_two_minors_vs_queen(
    ...
    def _state(board):
        ...
        bb = len(board.pieces(chess.BISHOP, chess.BLACK))
        bn = len(board.pieces(chess.KNIGHT, chess.BLACK))
        if wq == 1 and wr == 0 and (wb + wn) == 2 and bq == 1 and br == 0 and bb == 0 and bn == 0:
            return "white_q2m"
        if bq == 1 and br == 0 and (bb + bn) == 2 and wq == 1 and wr == 0 and wb == 0 and wn == 0:
            return "black_q2m"
        return None
    ...
    mover_name = "White" if mover_color == chess.WHITE else "Black"
    if state_after == "white_q2m":
        queen_two_minor_side = "White"
        lone_queen_side = "Black"
    else:
        queen_two_minor_side = "Black"
        lone_queen_side = "White"

    return True, {
        "queen_two_minor_side": queen_two_minor_side,
        "lone_queen_side": lone_queen_side,
        "mover": mover_name,
        "evidence": (
            f"{mover_name}'s move creates a queen-and-two-minors vs. lone-queen ending, "
            f"with {queen_two_minor_side} holding queen plus two minor pieces against "
            f"{lone_queen_side}'s lone queen — the two extra minor pieces give "
            f"{queen_two_minor_side} a decisive material advantage."
        ),
    }
...
GATED_TAGS = (
    ...
    "queen_vs_rook_and_two_minors",
    "rook_and_minor_vs_minor_pair",
    "queen_and_rook_vs_rook_and_minor",
    "queen_and_minor_vs_rook_and_minor",
    "two_rooks_vs_rook_and_minor",
    "queen_and_two_minors_vs_queen",
...
def certified_claims(
    ...
    rmvr = _safe(lambda: has_rook_and_minor_vs_rook(board_before, move, board_after, mover_color))
    if rmvr and rmvr[0]:
        tags.add("rook_and_minor_vs_rook")

    qvr2m = _safe(lambda: has_queen_vs_rook_and_two_minors(board_before, move, board_after, mover_color))
    if qvr2m and qvr2m[0]:
        tags.add("queen_vs_rook_and_two_minors")

    rm2m = _safe(lambda: has_rook_and_minor_vs_minor_pair(board_before, move, board_after, mover_color))
    if rm2m and rm2m[0]:
        tags.add("rook_and_minor_vs_minor_pair")

    qrvram = _safe(lambda: has_queen_and_rook_vs_rook_and_minor(board_before, move, board_after, mover_color))
    if qrvram and qrvram[0]:
        tags.add("queen_and_rook_vs_rook_and_minor")

    qmvram = _safe(lambda: has_queen_and_minor_vs_rook_and_minor(board_before, move, board_after, mover_color))
    if qmvram and qmvram[0]:
        tags.add("queen_and_minor_vs_rook_and_minor")

    trvram = _safe(lambda: has_two_rooks_vs_rook_and_minor(board_before, move, board_after, mover_color))
    if trvram and trvram[0]:
        tags.add("two_rooks_vs_rook_and_minor")

    qt2mvq = _safe(lambda: has_queen_and_two_minors_vs_queen(board_before, move, board_after, mover_color))
    if qt2mvq and qt2mvq[0]:
        tags.add("queen_and_two_minors_vs_queen")
