...
### Added
- **`has_queen_and_two_minors_vs_queen` factgate predicate** (`queen_and_two_minors_vs_queen` tag) — fires when a move newly creates a queen-and-two-minors vs. lone-queen imbalance: one side has exactly one queen and two minor pieces (no rooks), the other has exactly one queen and no other pieces. Set-difference via inner `_state()` returning 'white_q2m'/'black_q2m'/None. Evidence carries `queen_two_minor_side`, `lone_queen_side`, and `mover`; commentary notes the decisive material advantage of the extra two minor pieces. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 801 passed.

## [0.41.99] — 2026-06-19

### Added
- **`has_two_rooks_vs_rook_and_minor` factgate predicate** (`two_rooks_vs_rook_and_minor` tag) — fires when a move newly creates a two-rooks vs. rook-and-minor imbalance: one side has exactly two rooks (no queen, no minors), the other has exactly one rook and one minor piece (no queen). Set-difference via inner `_state()` returning 'white_2r'/'black_2r'/None. Evidence carries `two_rook_side`, `rook_minor_side`, and `mover`; commentary notes the rook pair's material edge and open-file coordination advantage. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 795 passed.

## [0.41.98] — 2026-06-19

### Added
- **`has_queen_and_minor_vs_rook_and_minor` factgate predicate** (`queen_and_minor_vs_rook_and_minor` tag) — fires when a move newly creates a queen-and-minor vs. rook-and-minor imbalance: one side has exactly one queen and one minor piece (no rooks), the other has exactly one rook and one minor piece (no queen). Set-difference via inner `_state()` returning 'white_qm'/'black_qm'/None. Evidence carries `queen_minor_side`, `rook_minor_side`, and `mover`; commentary notes the queen's positional and material superiority. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 789 passed.

## [0.41.97] — 2026-06-19

### Added
- **`has_queen_and_rook_vs_rook_and_minor` factgate predicate** (`queen_and_rook_vs_rook_and_minor` tag) — fires when a move newly creates a queen-and-rook vs. rook-and-minor imbalance: one side has exactly one queen and one rook (no minors), the other has exactly one rook and one minor piece (no queen). Set-difference via inner `_state()` returning 'white_qr'/'black_qr'/None. Evidence carries `queen_rook_side`, `rook_minor_side`, and `mover`; commentary notes the decisive extra-queen advantage. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 783 passed.

## [0.41.96] — 2026-06-19

### Added
- **`has_rook_and_minor_vs_minor_pair` factgate predicate** (`rook_and_minor_vs_minor_pair` tag) — fires when a move newly creates a rook-and-minor vs. two-minor-pieces imbalance: one side has exactly one rook and one minor piece (no queen), the other has exactly two minor pieces (no queen, no rook). Set-difference via inner `_state()` returning 'white_rm2m'/'black_rm2m'/None. Evidence carries `rook_minor_side`, `minor_pair_side`, and `mover`; commentary notes the rook's material edge and its growth as the position opens. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 777 passed.

## [0.41.95] — 2026-06-19

### Added
- **`has_queen_vs_rook_and_two_minors` factgate predicate** (`queen_vs_rook_and_two_minors` tag) — fires when a move newly creates a queen vs. rook-and-two-minors imbalance: one side has exactly one queen (no rooks, no minors), the other has exactly one rook and two minor pieces in any B/N combination (no queen). Set-difference via inner `_state()` returning 'white_queen'/'black_queen'/None. Evidence carries `queen_side`, `rook_two_minor_side`, and `mover`; commentary notes the material deficit on the queen side and the need for immediate activity. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 771 passed.
...
### Added
- **`has_queen_and_rook_vs_queen` factgate predicate** (`queen_and_rook_vs_queen` tag) — fires when a move newly creates a queen-and-rook vs. lone-queen imbalance: one side has exactly one queen and one rook (no minors), the other has exactly one queen (no rooks, no minors). Set-difference via inner `_state()` returning 'white_qr'/'black_qr'/None. Evidence carries `queen_rook_side`, `lone_queen_side`, and `mover`; commentary notes the decisive extra-rook advantage and checks for perpetual/fortress saving resources. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 753 passed.

## [0.41.91] — 2026-06-19

### Added
- **`has_three_minors_vs_rook` factgate predicate** (`three_minors_vs_rook` tag) — fires when a move newly creates a three-minor-pieces vs. lone-rook imbalance: one side has exactly three minor pieces in any B/N combination (no rooks, no queen), the other has exactly one rook (no queen, no minors). Set-difference via inner `_state()` returning 'white_minors'/'black_minors'/None. Evidence carries `three_minor_side`, `rook_side`, and `mover`; commentary notes the decisive material advantage and coordination requirement. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 747 passed.

## [0.41.90] — 2026-06-19

### Added
- **`has_queen_vs_two_rooks_and_minor` factgate predicate** (`queen_vs_two_rooks_and_minor` tag) — fires when a move newly creates a queen vs. two-rooks-and-minor imbalance: one side has exactly one queen (no rooks, no minors), the other has exactly two rooks and one minor piece (no queen). Set-difference via inner `_state()` returning 'white_queen'/'black_queen'/None. Evidence carries `queen_side`, `two_rook_minor_side`, and `mover`; commentary notes that the rook pair plus minor typically outweighs a lone queen and the queen must create immediate counterplay. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 741 passed.

## [0.41.89] — 2026-06-19

### Added
- **`has_two_rooks_vs_three_minors` factgate predicate** (`two_rooks_vs_three_minors` tag) — fires when a move newly creates a two-rooks vs. three-minor-pieces imbalance: one side has exactly two rooks (no queen, no minors), the other has exactly three minor pieces in any B/N combination (no queen, no rooks). Set-difference via inner `_state()` returning 'white_rooks'/'black_rooks'/None. Evidence carries `two_rook_side`, `three_minor_side`, and `mover`; commentary contrasts open-file rook domination against closed-position minor-piece coordination. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 735 passed.
...
### Added
- **`has_rook_and_two_minors_vs_two_rooks` factgate predicate** (`rook_and_two_minors_vs_two_rooks` tag) — fires when a move newly creates a rook-and-two-minors vs. two-rooks imbalance: one side has exactly one rook and two minor pieces (no queen), the other has exactly two rooks (no queen, no minors). Set-difference via inner `_state()` returning 'white_r2m'/'black_r2m'/None. Evidence carries `rook_minor_side`, `two_rook_side`, and `mover`; commentary notes the minor pieces compensating through outpost control and coordination. Companion to `rook_and_minor_vs_two_rooks`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 723 passed.
...
### Added
- **`has_queen_and_minor_vs_two_rooks` factgate predicate** (`queen_and_minor_vs_two_rooks` tag) — fires when a move newly creates a queen-and-minor vs. two-rooks imbalance: one side has exactly one queen and one minor piece (no rooks), the other has exactly two rooks (no queen, no minors). Set-difference via inner `_state()` returning 'white_qm'/'black_qm'/None. Evidence carries `queen_minor_side`, `two_rook_side`, and `mover`; commentary emphasises the queen as dominant coordinator against a compact rook pair, with the minor piece adding tactical threats. Companion to `rook_and_minor_vs_two_rooks`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 711 passed.

## [0.41.84] — 2026-06-19

### Added
- **`has_queen_vs_knight` factgate predicate** (`queen_vs_knight` tag) — fires when a move newly creates a queen-vs-lone-knight imbalance: one side has exactly one queen (no rooks, no minors), the other has exactly one knight (no queens, no rooks, no bishops). Set-difference via inner `_state()` returning 'white_queen'/'black_queen'/None. Evidence carries `queen_side`, `knight_side`, and `mover`; commentary notes the queen's ~6-pawn edge and the knight's particular difficulty defending against long-range queen threats. Companion to `queen_vs_bishop`; completes the queen-vs-single-piece family. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 705 passed.
...
### Added
- **`has_two_rooks_vs_minor_pair` factgate predicate** (`two_rooks_vs_minor_pair` tag) — fires when a move newly creates a two-rooks-vs-minor-pair imbalance: one side has exactly two rooks (no queens, no minors), the other has exactly two minor pieces (any B/N combination, no rooks, no queens). Set-difference via inner `_state()` returning 'white_rooks'/'black_rooks'/None; uses `(bb+bn)==2`/`(wb+wn)==2` to cover all minor combinations. Evidence carries `two_rook_side`, `minor_pair_side`, and `mover`. Narrator rule appended to line 202. Wired into `GATED_TAGS`, `certified_claims()`. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 693 passed.

## [0.41.81] — 2026-06-19

### Added
- **`has_rook_and_minor_vs_two_rooks` factgate predicate** (`rook_and_minor_vs_two_rooks` tag) — fires when a move newly creates a rook-and-minor vs. two rooks imbalance: one side has exactly one rook and exactly one minor piece (no queens), the other has exactly two rooks (no queens, no minors). Set-difference via inner `_state()` returning 'white_rook_minor'/'black_rook_minor'/None; uses `(wb+wn)==1`/`(bb+bn)==1` to cover R+B and R+N cases. Evidence carries `minor_rook_side`, `two_rook_side`, and `mover`. Narrator rule appended to line 202. Wired into `GATED_TAGS`, `certified_claims()`. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 687 passed.

## [0.41.80] — 2026-06-19

### Added
- **`has_queen_vs_two_minors` factgate predicate** (`queen_vs_two_minors` tag) — fires when a move newly creates a queen-vs-two-minors imbalance: one side has exactly one queen (no rooks, no minors), the other has exactly two minor pieces (any combination of bishops/knights, no rooks, no queens). Set-difference via inner `_state()` returning 'white_queen'/'black_queen'/None; uses `(bb+bn)==2`/`(wb+wn)==2` to cover all minor-piece combinations. Evidence carries `queen_side`, `minor_side`, and `mover`. Narrator rule appended to line 202. Wired into `GATED_TAGS`, `certified_claims()`. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 681 passed.

## [0.41.79] — 2026-06-19

### Added
- **`has_queen_vs_rook` factgate predicate** (`queen_vs_rook` tag) — fires when a move newly creates a queen-vs-lone-rook imbalance: one side has exactly one queen (no rooks, bishops, or knights), the other has exactly one rook (no queens, bishops, or knights). Set-difference via inner `_state()` returning 'white_queen'/'black_queen'/None. Evidence carries `queen_side`, `rook_side`, and `mover`; commentary frames the queen's ~2-pawn edge while calibrating to fortress and perpetual-check drawing resources for the rook side. Extends the material-imbalance predicate series; complements `two_rooks_vs_queen`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 675 passed.
...
### Added
- **`has_rook_vs_knight` factgate predicate** (`rook_vs_knight` tag) — fires when a move newly creates a clean rook-vs-knight imbalance: one side has exactly one rook (no queens, no bishops, no knights), the other has exactly one knight (no queens, no rooks, no bishops). Set-difference via inner `_state()` returning 'white_rook'/'black_rook'/None. Evidence carries `rook_side`, `knight_side`, and `mover`; commentary notes the rook's ~2-pawn theoretical edge while acknowledging the knight's leaping resilience in closed/fixed pawn structures. Companion to `rook_vs_bishop`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 657 passed.

## [0.41.75] — 2026-06-19

### Added
- **`has_rook_vs_bishop` factgate predicate** (`rook_vs_bishop` tag) — fires when a move newly creates a clean rook-vs-bishop imbalance: one side has exactly one rook (no queens, no minors), the other has exactly one bishop (no queens, no rooks, no other minors). Set-difference via inner `_state()` returning 'white_rook'/'black_rook'/None. Evidence carries `rook_side`, `bishop_side`, and `mover`; commentary frames the rook's ~2-pawn advantage while noting the bishop's drawing chances in blocked positions. Continues the material-imbalance predicate series. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 651 passed.
...
### Added
- **`has_rook_vs_two_minors` factgate predicate** (`rook_vs_two_minors` tag) — fires when a move newly creates a rook-vs-two-minor-pieces imbalance: one side has exactly one rook (no queens, no minors), the other has exactly two minor pieces in any combination (no queens, no rooks). Set-difference guards prevent re-firing. Inner `_state()` returns 'white_rook'/'black_rook'/None; uses `wm = wb + wn` / `bm = bb + bn` for minor counts. Evidence carries `rook_side`, `minor_side`, and `mover`; commentary notes the two-piece theoretical edge (~0.5 pawn) while calibrating to the rook's practical open-file and penetration chances. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 639 passed.

## [0.41.72] — 2026-06-19

### Added
- **`has_queen_vs_rook_and_minor` factgate predicate** (`queen_vs_rook_and_minor` tag) — fires when a move newly creates a pure queen vs. rook-and-minor imbalance: one side has exactly one queen (no rooks, no minor pieces), the other has exactly one rook and one minor piece (no queens). Set-difference guards against re-firing. Inner `_state()` returns 'white_queen'/'black_queen'/None; uses `wm = wb + wn` / `bm = bb + bn` to count minors. Evidence carries `queen_side`, `rook_side`, and `mover`; commentary acknowledges the queen's slight theoretical edge (~0.5 pawn) while calibrating to practical compensation in dynamic positions with an active rook and strong minor outpost. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 633 passed.

## [0.41.71] — 2026-06-19

### Added
- **`has_two_rooks_vs_queen` factgate predicate** (`two_rooks_vs_queen` tag) — fires when a move newly creates a pure 2R vs 1Q imbalance: one side has exactly two rooks and zero queens, the other exactly one queen and zero rooks. Set-difference guards against re-firing. Inner `_state()` returns 'white_rooks'/'black_rooks'/None. Evidence carries `rook_side`, `queen_side`, and `mover`; commentary calibrates to open vs. closed positions (rooks thrive in open files, queen dominates in closed). Mirrors pattern of `has_two_bishops_vs_two_knights`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 627 passed.
...
### Added
- **`has_rook_on_back_rank` factgate predicate** (`rook_on_back_rank` tag) — fires when the mover's rook newly lands on the opponent's back rank (rank idx 7 for White, idx 0 for Black), where the rook was not already on that rank. Checks moving piece is a rook, landing rank matches target, and from-rank differs. The logical endpoint of the rook-penetration motif (sixth→seventh→back rank). Evidence carries `square`, `rank` ('eighth'/'first'), and `mover`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 549 passed.
...
### Added
- **`rook_behind_own_passer` factgate predicate** (`rook_behind_passer` tag) — rook-move predicate certifying that the mover's rook just moved to a square directly behind one of its own passed pawns on the same file (Tarrasch rule). Guards on rook piece type, same-file check, directional behind-check, and `is_passed_pawn`. Evidence carries `rook`, `pawn`, `file`, `mover`, and a ready-to-quote `evidence` string. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt with Tarrasch-principle and king-cutoff pedagogy. 6 new tests (2 TRUE / 3 FALSE / 1 integration); full suite 381 passed.
...
### Added
- **`is_castling` factgate predicate** (`castling` tag) — move predicate certifying a castling move, with evidence `{side, color}` ('kingside'/'queenside', 'White'/'Black'). Uses `board.is_castling()` and `board.is_kingside_castling()`. Wired into `GATED_TAGS`, `certified_claims()`, and narrator system prompt with rook-jump mechanics and king-safety framing. 5 new pure tests; full suite 266 passed.
