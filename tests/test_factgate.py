...
# ---------------------------------------------------------------------------
# has_rook_vs_two_minors
# ---------------------------------------------------------------------------

def _rvtm(fen, uci, col):
    bb = chess.Board(fen)
    mv = chess.Move.from_uci(uci)
    ba = bb.copy()
    ba.push(mv)
    return F.has_rook_vs_two_minors(bb, mv, ba, col)


def test_rook_vs_two_minors_black_takes_white_knight_true():
    # Black Nf3 captures White Ng1; after: White has R only, Black has B+N
    ok, ev = _rvtm("6k1/8/8/2b5/8/5n2/8/R5NK b - - 0 1", "f3g1", chess.BLACK)
    assert ok
    assert ev["rook_side"] == "White"
    assert ev["minor_side"] == "Black"


def test_rook_vs_two_minors_white_takes_black_bishop_true():
    # White Nc4 captures Black Bd6; after: Black has R only, White has B+N
    ok, ev = _rvtm("3r2k1/8/3b4/8/2N5/B7/8/4K3 w - - 0 1", "c4d6", chess.WHITE)
    assert ok
    assert ev["rook_side"] == "Black"
    assert ev["minor_side"] == "White"


def test_rook_vs_two_minors_imbalance_does_not_apply_false():
    # White has R+N, Black has R — no imbalance after Ra5
    ok, _ = _rvtm("r3k3/8/8/8/8/8/8/RN2K3 w - - 0 1", "a1a5", chess.WHITE)
    assert not ok


def test_rook_vs_two_minors_imbalance_pre_existed_false():
    # Before: White has R, Black has N+B — imbalance already present; Ra5 does not create it
    ok, _ = _rvtm("1nbk4/8/8/8/8/8/8/R3K3 w - - 0 1", "a1a5", chess.WHITE)
    assert not ok


def test_rook_vs_two_minors_three_minors_on_one_side_false():
    # Black has N+N+B (3 minors) — does not match the 2-minor condition
    ok, _ = _rvtm("1nnk4/8/8/2b5/8/8/8/R3K3 w - - 0 1", "a1a5", chess.WHITE)
    assert not ok


def test_rook_vs_two_minors_in_certified_claims():
    fen = "6k1/8/8/2b5/8/5n2/8/R5NK b - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("f3g1")
    board_after = board_before.copy()
    board_after.push(move)
...
def _rvb(fen, uci, col):
    bb = chess.Board(fen)
    mv = chess.Move.from_uci(uci)
    ba = bb.copy()
    ba.push(mv)
    return F.has_rook_vs_bishop(bb, mv, ba, col)


def test_rook_vs_bishop_white_rook_captures_black_knight_true():
    # White Re1 captures Black Ne4; after: White has R only, Black has B only
    ok, ev = _rvb("2bk4/8/8/8/4n3/8/8/4RK2 w - - 0 1", "e1e4", chess.WHITE)
    assert ok
    assert ev["rook_side"] == "White"
    assert ev["bishop_side"] == "Black"


def test_rook_vs_bishop_black_rook_captures_white_knight_true():
    # Black Rb2 captures White Nb1; after: Black has R only, White has B only
    ok, ev = _rvb("4k3/8/8/8/8/8/1r6/BN2K3 b - - 0 1", "b2b1", chess.BLACK)
    assert ok
    assert ev["rook_side"] == "Black"
    assert ev["bishop_side"] == "White"


def test_rook_vs_bishop_extra_minor_on_board_false():
    # Black still has B+N after Ra5; no 1R:1B imbalance
    ok, _ = _rvb("2bnk3/8/8/8/8/8/8/R3K3 w - - 0 1", "a1a5", chess.WHITE)
    assert not ok


def test_rook_vs_bishop_imbalance_pre_existed_false():
    # Already 1R vs 1B before the move
    ok, _ = _rvb("2bk4/8/8/8/8/8/8/R3K3 w - - 0 1", "a1a5", chess.WHITE)
    assert not ok


def test_rook_vs_bishop_rook_side_also_has_bishop_false():
    # White has R+B after capturing Black knight — not a clean 1R vs 1B imbalance
    ok, _ = _rvb("4k3/8/8/8/3n4/8/8/2BRK3 w - - 0 1", "d1d4", chess.WHITE)
    assert not ok


def test_rook_vs_bishop_in_certified_claims():
    fen = "2bk4/8/8/8/4n3/8/8/4RK2 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("e1e4")
    board_after = board_before.copy()
    board_after.push(move)
...
# ---------------------------------------------------------------------------
# has_two_bishops_vs_rook
# ---------------------------------------------------------------------------

def _tbvr(fen, uci, col):
    bb = chess.Board(fen)
    mv = chess.Move.from_uci(uci)
    ba = bb.copy()
    ba.push(mv)
    return F.has_two_bishops_vs_rook(bb, mv, ba, col)


def test_two_bishops_vs_rook_white_bishops_capture_black_knight_true():
    # White Bc3 captures Black Ne5; after: White has 2B, Black has R only
    ok, ev = _tbvr("4k3/8/8/4n3/8/2B3B1/3r4/4K3 w - - 0 1", "c3e5", chess.WHITE)
    assert ok
    assert ev["bishop_side"] == "White"
    assert ev["rook_side"] == "Black"


def test_two_bishops_vs_rook_black_bishops_capture_white_knight_true():
    # Black Bd3 captures White Ne4; after: Black has 2B, White has R only
    ok, ev = _tbvr("4k3/8/8/8/4N3/3b4/8/b3KR2 b - - 0 1", "d3e4", chess.BLACK)
    assert ok
    assert ev["bishop_side"] == "Black"
    assert ev["rook_side"] == "White"


def test_two_bishops_vs_rook_extra_rook_piece_false():
    # White has R+B vs Black 2B — not clean 2B:1R
    ok, _ = _tbvr("4k3/8/8/8/8/8/2bb4/R1B1K3 w - - 0 1", "a1a5", chess.WHITE)
    assert not ok


def test_two_bishops_vs_rook_imbalance_pre_existed_false():
    # Already 2B vs R before the move
    ok, _ = _tbvr("4k3/8/8/8/8/8/2bb4/4KR2 w - - 0 1", "f1f5", chess.WHITE)
    assert not ok


def test_two_bishops_vs_rook_bishop_side_has_knight_too_false():
    # Black has 2B+N; not a clean 2B:1R imbalance
    ok, _ = _tbvr("4k3/8/8/8/4n3/8/2bb4/4KR2 w - - 0 1", "f1f5", chess.WHITE)
    assert not ok


def test_two_bishops_vs_rook_in_certified_claims():
    fen = "4k3/8/8/4n3/8/2B3B1/3r4/4K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("c3e5")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
...
# ── has_rook_and_minor_vs_two_rooks ──────────────────────────────────────────

def _rmvtr(fen, uci, color):
    b = chess.Board(fen)
    mv = chess.Move.from_uci(uci)
    ba = b.copy(); ba.push(mv)
    return F.has_rook_and_minor_vs_two_rooks(b, mv, ba, color)


def test_rook_and_minor_vs_two_rooks_white_side_true():
    # Bg1xNd4 captures Black knight → White R+B vs Black 2R
    ok, d = _rmvtr("1r2k3/8/8/8/r2n4/8/8/4K1BR w - - 0 1", "g1d4", chess.WHITE)
    assert ok
    assert d["minor_rook_side"] == "White"
    assert d["two_rook_side"] == "Black"


def test_rook_and_minor_vs_two_rooks_black_side_true():
    # Bg1xNd4 (Black bishop) captures White knight → Black R+B vs White 2R
    ok, d = _rmvtr("1R2k3/8/8/8/R2N4/8/8/4K1br b - - 0 1", "g1d4", chess.BLACK)
    assert ok
    assert d["minor_rook_side"] == "Black"
    assert d["two_rook_side"] == "White"


def test_rook_and_minor_vs_two_rooks_rook_minor_side_has_extra_minor_false():
    # White has R+B+N after capture — not a clean R+minor
    ok, _ = _rmvtr("1r2k3/8/8/8/r2n4/8/8/3RK1BN w - - 0 1", "g1d4", chess.WHITE)
    assert not ok


def test_rook_and_minor_vs_two_rooks_imbalance_pre_existed_false():
    # Already R+B vs 2R before the move
    ok, _ = _rmvtr("1r2k3/8/8/8/r7/8/8/4K1BR w - - 0 1", "h1h3", chess.WHITE)
    assert not ok


def test_rook_and_minor_vs_two_rooks_two_rook_side_has_minor_false():
    # Black still has a knight (not clean 2R) after the move
    ok, _ = _rmvtr("1r2k3/8/8/8/r2n4/8/8/4K1BR w - - 0 1", "h1h3", chess.WHITE)
    assert not ok


def test_rook_and_minor_vs_two_rooks_in_certified_claims():
    fen = "1r2k3/8/8/8/r2n4/8/8/4K1BR w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("g1d4")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
...
def _qvk(fen, uci, color):
    b = chess.Board(fen)
    mv = chess.Move.from_uci(uci)
    ba = b.copy(); ba.push(mv)
    return F.has_queen_vs_knight(b, mv, ba, color)


def test_queen_vs_knight_white_queen_true():
    # Qe4xBd5 captures Black bishop → White Q vs Black N
    ok, d = _qvk("3nk3/8/8/3b4/4Q3/8/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert ok
    assert d["queen_side"] == "White"
    assert d["knight_side"] == "Black"


def test_queen_vs_knight_black_queen_true():
    # Qe4xBd5 (Black queen) captures White bishop → Black Q vs White N
    ok, d = _qvk("4k3/8/8/3B4/4q3/8/8/3NK3 b - - 0 1", "e4d5", chess.BLACK)
    assert ok
    assert d["queen_side"] == "Black"
    assert d["knight_side"] == "White"


def test_queen_vs_knight_knight_side_still_has_bishop_false():
    # Black retains bishop AND knight — not clean Q:N
    ok, _ = _qvk("3nk3/8/8/3bb3/4Q3/8/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_vs_knight_imbalance_pre_existed_false():
    # Already Q vs N before the move
    ok, _ = _qvk("3nk3/8/8/8/4Q3/8/8/4K3 w - - 0 1", "e4e5", chess.WHITE)
    assert not ok


def test_queen_vs_knight_queen_side_has_rook_false():
    # White has Q+R after capture — not clean Q
    ok, _ = _qvk("3nk3/8/8/3b4/4Q3/8/8/4KR2 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_vs_knight_in_certified_claims():
    fen = "3nk3/8/8/3b4/4Q3/8/8/4K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("e4d5")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
    assert "queen_vs_knight" in tags


# ── has_queen_and_minor_vs_two_rooks ─────────────────────────────────────


def _qmvtr(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_queen_and_minor_vs_two_rooks(board_before, move, board_after, mover_color)


def test_queen_and_minor_vs_two_rooks_white_queen_bishop_true():
    # White Qe4 captures Black Nd5 → White Q+B vs Black 2R (newly created)
    ok, info = _qmvtr("1r2k3/8/8/3n4/r3Q3/3B4/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert ok
    assert info["queen_minor_side"] == "White"
    assert info["two_rook_side"] == "Black"


def test_queen_and_minor_vs_two_rooks_black_queen_bishop_true():
    # Black Qd4 captures White Nd5 → Black Q+B vs White 2R (newly created)
    ok, info = _qmvtr("1R2k3/8/8/3N4/R2q3b/8/8/4K3 b - - 0 1", "d4d5", chess.BLACK)
    assert ok
    assert info["queen_minor_side"] == "Black"
    assert info["two_rook_side"] == "White"


def test_queen_and_minor_vs_two_rooks_queen_side_has_rook_false():
    # White has Q+R+B after capture — not clean Q+minor
    ok, _ = _qmvtr("1r2k3/8/8/3n4/r3Q3/3B1R2/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_and_minor_vs_two_rooks_imbalance_pre_existed_false():
    # Already Q+B vs 2R before the move — set-difference must not fire
    ok, _ = _qmvtr("1r2k3/8/8/8/r3Q3/3B4/8/4K3 w - - 0 1", "e4e5", chess.WHITE)
    assert not ok


def test_queen_and_minor_vs_two_rooks_rook_side_has_extra_false():
    # Rook side retains an extra bishop — not clean 2R
    ok, _ = _qmvtr("1r2k3/8/4b3/3n4/r3Q3/3B4/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_and_minor_vs_two_rooks_in_certified_claims():
    fen = "1r2k3/8/8/3n4/r3Q3/3B4/8/4K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("e4d5")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
    assert "queen_and_minor_vs_two_rooks" in tags


# ── has_queen_and_rook_vs_two_rooks ──────────────────────────────────────


def _qrv2r(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_queen_and_rook_vs_two_rooks(board_before, move, board_after, mover_color)


def test_queen_and_rook_vs_two_rooks_white_true():
    # White Qe4 captures Black Nd5 → White Q+R vs Black 2R (newly created)
    ok, info = _qrv2r("1r2k3/8/8/3n4/r3Q3/4R3/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert ok
    assert info["queen_rook_side"] == "White"
    assert info["two_rook_side"] == "Black"


def test_queen_and_rook_vs_two_rooks_black_true():
    # Black Qd4 captures White Nd5 → Black Q+R vs White 2R (newly created)
    ok, info = _qrv2r("1R2k3/8/8/3N4/R2q4/4r3/8/4K3 b - - 0 1", "d4d5", chess.BLACK)
    assert ok
    assert info["queen_rook_side"] == "Black"
    assert info["two_rook_side"] == "White"


def test_queen_and_rook_vs_two_rooks_queen_side_has_minor_false():
    # White has Q+R+B after capture — not clean Q+R
    ok, _ = _qrv2r("1r2k3/8/8/3n4/r3Q3/4RB2/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_and_rook_vs_two_rooks_imbalance_pre_existed_false():
    # Already Q+R vs 2R before the move — set-difference must not fire
    ok, _ = _qrv2r("1r2k3/8/8/8/r3Q3/4R3/8/4K3 w - - 0 1", "e4f5", chess.WHITE)
    assert not ok


def test_queen_and_rook_vs_two_rooks_rook_side_has_extra_false():
    # Rook side retains an extra bishop — not clean 2R
    ok, _ = _qrv2r("1r2k3/4b3/8/3n4/r3Q3/4R3/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_and_rook_vs_two_rooks_in_certified_claims():
    fen = "1r2k3/8/8/3n4/r3Q3/4R3/8/4K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("e4d5")
    board_after = board_before.copy()
    board_after.push(move)
...
# ── has_queen_vs_three_minors ─────────────────────────────────────────────


def _qv3m(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_queen_vs_three_minors(board_before, move, board_after, mover_color)


def test_queen_vs_three_minors_white_queen_true():
    # White Qe4 captures Black Ra4 → White Q vs Black B+N+N (newly created)
    ok, info = _qv3m("4k3/8/8/1b1nn3/r3Q3/8/8/4K3 w - - 0 1", "e4a4", chess.WHITE)
    assert ok
    assert info["queen_side"] == "White"
    assert info["three_minor_side"] == "Black"


def test_queen_vs_three_minors_black_queen_true():
    # Black Qe5 captures White Ra5 → Black Q vs White B+N+N (newly created)
    ok, info = _qv3m("4k3/8/8/R3q3/1BNN4/8/8/4K3 b - - 0 1", "e5a5", chess.BLACK)
    assert ok
    assert info["queen_side"] == "Black"
    assert info["three_minor_side"] == "White"


def test_queen_vs_three_minors_queen_side_has_rook_false():
    # White has Q+R — not clean queen
    ok, _ = _qv3m("4k3/8/8/1b1nn3/r3Q3/8/8/3RK3 w - - 0 1", "e4a4", chess.WHITE)
    assert not ok


def test_queen_vs_three_minors_imbalance_pre_existed_false():
    # Already Q vs 3 minors before the move — set-difference must not fire
    ok, _ = _qv3m("4k3/8/8/1b1nn3/4Q3/8/8/4K3 w - - 0 1", "e4f5", chess.WHITE)
    assert not ok


def test_queen_vs_three_minors_minor_side_has_queen_false():
    # Minor side also has a queen — not clean 3-minor army
    ok, _ = _qv3m("4k3/8/8/1b1nn3/r3Q3/8/8/3qK3 w - - 0 1", "e4a4", chess.WHITE)
    assert not ok


def test_queen_vs_three_minors_in_certified_claims():
    fen = "4k3/8/8/1b1nn3/r3Q3/8/8/4K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("e4a4")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
    assert "queen_vs_three_minors" in tags


# ── has_two_rooks_vs_three_minors ─────────────────────────────────────────


def _trvtm(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_two_rooks_vs_three_minors(board_before, move, board_after, mover_color)


def test_two_rooks_vs_three_minors_white_rooks_true():
    # White Ra1 captures Black Ra4 → White 2R vs Black B+N+N (newly created)
    ok, info = _trvtm("4k3/8/8/1b1nn3/r7/8/8/RR2K3 w - - 0 1", "a1a4", chess.WHITE)
    assert ok
    assert info["two_rook_side"] == "White"
    assert info["three_minor_side"] == "Black"


def test_two_rooks_vs_three_minors_black_rooks_true():
    # Black Ra8 captures White Ra5 → Black 2R vs White B+N+N (newly created)
    ok, info = _trvtm("rr2k3/8/8/R7/1BNN4/8/8/4K3 b - - 0 1", "a8a5", chess.BLACK)
    assert ok
    assert info["two_rook_side"] == "Black"
    assert info["three_minor_side"] == "White"


def test_two_rooks_vs_three_minors_rook_side_has_queen_false():
    # Rook side also has a queen — not clean 2R
    ok, _ = _trvtm("4k3/8/8/1b1nn3/r7/8/8/RRQ1K3 w - - 0 1", "a1a4", chess.WHITE)
    assert not ok


def test_two_rooks_vs_three_minors_imbalance_pre_existed_false():
    # Already 2R vs 3 minors before the move — set-difference must not fire
    ok, _ = _trvtm("4k3/8/8/1b1nn3/8/8/8/RR2K3 w - - 0 1", "a1a3", chess.WHITE)
    assert not ok


def test_two_rooks_vs_three_minors_minor_side_has_extra_rook_false():
    # Minor side retains an extra rook after capture — not clean 3 minors
    ok, _ = _trvtm("4k3/2r5/8/1b1nn3/r7/8/8/RR2K3 w - - 0 1", "a1a4", chess.WHITE)
    assert not ok


def test_two_rooks_vs_three_minors_in_certified_claims():
    fen = "4k3/8/8/1b1nn3/r7/8/8/RR2K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("a1a4")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
    assert "two_rooks_vs_three_minors" in tags


# ── has_queen_vs_two_rooks_and_minor ──────────────────────────────────────


def _qv2rm(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_queen_vs_two_rooks_and_minor(board_before, move, board_after, mover_color)


def test_queen_vs_two_rooks_and_minor_white_queen_true():
    # White Qe4 captures Black Nd5 → White Q vs Black 2R+N (newly created)
    ok, info = _qv2rm("1r2k3/8/8/1n1n4/r3Q3/8/8/4K3 w - - 0 1", "e4d5", chess.WHITE)
    assert ok
    assert info["queen_side"] == "White"
    assert info["two_rook_minor_side"] == "Black"


def test_queen_vs_two_rooks_and_minor_black_queen_true():
    # Black Qe4 captures White Nd5 → Black Q vs White 2R+N (newly created)
    ok, info = _qv2rm("4k3/8/8/1N1N4/R3q3/8/8/1R2K3 b - - 0 1", "e4d5", chess.BLACK)
    assert ok
    assert info["queen_side"] == "Black"
    assert info["two_rook_minor_side"] == "White"


def test_queen_vs_two_rooks_and_minor_queen_side_has_rook_false():
    # White has Q+R after capture — not clean queen
    ok, _ = _qv2rm("1r2k3/8/8/1n1n4/r3Q3/8/8/4KR2 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_vs_two_rooks_and_minor_imbalance_pre_existed_false():
    # Already Q vs 2R+minor before the move — set-difference must not fire
    ok, _ = _qv2rm("1r2k3/8/8/1n6/r3Q3/8/8/4K3 w - - 0 1", "e4f4", chess.WHITE)
    assert not ok


def test_queen_vs_two_rooks_and_minor_rook_minor_side_has_queen_false():
    # The 2R+minor side also has a queen — not clean 2R+minor
    ok, _ = _qv2rm("1r2k3/8/8/1n1n4/r3Q3/8/8/3qK3 w - - 0 1", "e4d5", chess.WHITE)
    assert not ok


def test_queen_vs_two_rooks_and_minor_in_certified_claims():
    fen = "1r2k3/8/8/1n1n4/r3Q3/8/8/4K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("e4d5")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
    assert "queen_vs_two_rooks_and_minor" in tags


# ── has_three_minors_vs_rook ──────────────────────────────────────────────


def _tmvr(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_three_minors_vs_rook(board_before, move, board_after, mover_color)


def test_three_minors_vs_rook_white_minors_true():
    # White Bb2 captures Black Ra1 → White B+N+N vs Black 1R (newly created)
    ok, info = _tmvr("4k3/8/8/8/r7/2NN4/1B6/r3K3 w - - 0 1", "b2a1", chess.WHITE)
    assert ok
    assert info["three_minor_side"] == "White"
    assert info["rook_side"] == "Black"


def test_three_minors_vs_rook_black_minors_true():
    # Black Bb7 captures White Ra8 → Black B+N+N vs White 1R (newly created)
    ok, info = _tmvr("R3k3/1b6/2nn4/8/8/8/R7/4K3 b - - 0 1", "b7a8", chess.BLACK)
    assert ok
    assert info["three_minor_side"] == "Black"
    assert info["rook_side"] == "White"


def test_three_minors_vs_rook_minor_side_has_rook_false():
    # White has B+N+N+R after capture — not clean 3 minors
    ok, _ = _tmvr("4k3/8/8/8/r7/2NN4/1B6/r2RK3 w - - 0 1", "b2a1", chess.WHITE)
    assert not ok


def test_three_minors_vs_rook_imbalance_pre_existed_false():
    # Already 3 minors vs 1R before the move — set-difference must not fire
    ok, _ = _tmvr("4k3/8/8/8/r7/2NN4/1B6/4K3 w - - 0 1", "c3d5", chess.WHITE)
    assert not ok


def test_three_minors_vs_rook_rook_side_has_extra_false():
    # Rook side retains an extra bishop after capture — not clean 1R
    ok, _ = _tmvr("1r2k3/8/8/1b6/8/2NN4/1B6/r3K3 w - - 0 1", "b2a1", chess.WHITE)
    assert not ok


def test_three_minors_vs_rook_in_certified_claims():
    fen = "4k3/8/8/8/r7/2NN4/1B6/r3K3 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("b2a1")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
    assert "three_minors_vs_rook" in tags


# ── has_queen_and_rook_vs_queen ───────────────────────────────────────────


def _qrvq(fen, uci, mover_color):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_queen_and_rook_vs_queen(board_before, move, board_after, mover_color)


def test_queen_and_rook_vs_queen_white_true():
    # White Rd4 captures Black Bb4 → White Q+R vs Black Q (newly created)
    ok, info = _qrvq("1q2k3/8/8/8/1b1R4/4K3/8/5Q2 w - - 0 1", "d4b4", chess.WHITE)
    assert ok
    assert info["queen_rook_side"] == "White"
    assert info["lone_queen_side"] == "Black"


def test_queen_and_rook_vs_queen_black_true():
    # Black Rd4 captures White Bb4 → Black Q+R vs White Q (newly created)
    ok, info = _qrvq("5q2/8/8/8/1B1r4/4k3/8/1Q2K3 b - - 0 1", "d4b4", chess.BLACK)
    assert ok
    assert info["queen_rook_side"] == "Black"
    assert info["lone_queen_side"] == "White"


def test_queen_and_rook_vs_queen_queen_rook_side_has_minor_false():
    # White has Q+R+B after capture — not clean Q+R
    ok, _ = _qrvq("1q2k3/8/8/8/1b1R4/4K3/8/5QB1 w - - 0 1", "d4b4", chess.WHITE)
    assert not ok


def test_queen_and_rook_vs_queen_imbalance_pre_existed_false():
    # Already Q+R vs Q before the move — set-difference must not fire
    ok, _ = _qrvq("1q2k3/8/8/8/3R4/4K3/8/5Q2 w - - 0 1", "d4d5", chess.WHITE)
    assert not ok


def test_queen_and_rook_vs_queen_lone_queen_side_has_minor_false():
    # Lone-queen side also has a bishop — not clean lone queen
    ok, _ = _qrvq("1q2k3/8/8/3b4/1b1R4/4K3/8/5Q2 w - - 0 1", "d4b4", chess.WHITE)
    assert not ok


def test_queen_and_rook_vs_queen_in_certified_claims():
    fen = "1q2k3/8/8/8/1b1R4/4K3/8/5Q2 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("d4b4")
    board_after = board_before.copy()
    board_after.push(move)
    tags = F.certified_claims(board_before, move, board_after, chess.WHITE)
...
# ---------------------------------------------------------------------------
# has_rook_and_minor_vs_minor_pair
# ---------------------------------------------------------------------------

def _rm2m(fen, move_uci, mover_color=chess.WHITE):
    board_before = chess.Board(fen)
    move = chess.Move.from_uci(move_uci)
    board_after = board_before.copy()
    board_after.push(move)
    return F.has_rook_and_minor_vs_minor_pair(board_before, move, board_after, mover_color)


def test_rook_and_minor_vs_minor_pair_white_true():
    # White Rb1 captures black knight on b4 → White R+B vs Black 2N
    result = _rm2m("7k/8/3n4/2n5/1n6/8/8/1RBK4 w - - 0 1", "b1b4")
    fired, info = result
    assert fired
    assert info["rook_minor_side"] == "White"
    assert info["minor_pair_side"] == "Black"


def test_rook_and_minor_vs_minor_pair_black_true():
    # Black rb8 captures white Bishop on b4 → Black R+N vs White 2B
    result = _rm2m("1r2k3/8/8/2n5/1B6/B7/8/2B4K b - - 0 1", "b8b4", chess.BLACK)
    fired, info = result
    assert fired
    assert info["rook_minor_side"] == "Black"
    assert info["minor_pair_side"] == "White"


def test_rook_and_minor_vs_minor_pair_rook_minor_side_has_queen_false():
    # After capture: White has Q+R+B, not clean R+minor — does not fire
    result = _rm2m("7k/8/3n4/2n5/1n6/8/8/QRBK4 w - - 0 1", "b1b4")
    fired, _ = result
    assert not fired


def test_rook_and_minor_vs_minor_pair_imbalance_pre_existed_false():
    # White already has R+B vs Black 2N before the move
    result = _rm2m("7k/8/3n4/2n5/8/8/8/1RBK4 w - - 0 1", "b1b2")
    fired, _ = result
    assert not fired


def test_rook_and_minor_vs_minor_pair_minor_pair_side_has_rook_false():
    # Black also has a rook alongside its two knights — not a pure minor pair
    result = _rm2m("1r5k/8/3n4/2n5/8/8/8/1RBK4 w - - 0 1", "b1b2")
    fired, _ = result
    assert not fired


def test_rook_and_minor_vs_minor_pair_in_certified_claims():
    fen = "7k/8/3n4/2n5/1n6/8/8/1RBK4 w - - 0 1"
    board_before = chess.Board(fen)
    move = chess.Move.from_uci("b1b4")
    board_after = board_before.copy()
    board_after.push(move)
